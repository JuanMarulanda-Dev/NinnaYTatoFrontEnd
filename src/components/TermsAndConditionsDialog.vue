<template>
  <v-row justify="center">
    <v-dialog v-model="dialogTermsAndConditions" persistent width="600px">
      <v-card>
        <v-card-title>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" text @click="agree()"> Aceptar </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
...
export default {
  computed: {
    ...mapState(["dialogTermsAndConditions"]),
  },
  methods: {
    ...mapMutations(["SET_DIALOG_TERMS_AND_CONDITIONS"]),
    ...mapActions(["acceptTermsAndConditions"]),
    agree() {
      let result = this.acceptTermsAndConditions();
      if (result) {
        this.SET_DIALOG_TERMS_AND_CONDITIONS(false);
        // Mostrar mensaje de que los terminos y condiciones fueron aceptados
        this.showToastMessage(201, "Terminos y condiciones aceptados");
        // Actualizar la variable de session del usuario en el local storage
      }
    },
  },
};
